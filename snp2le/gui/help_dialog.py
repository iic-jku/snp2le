...
<li><b>Enforce passivity</b> / <b>Passivity target</b> (universal mode): a model is
...
often by one to two orders of magnitude in RMS error. The <b>Passivity target</b> is how
far the enforcement has to go, so it buys that accuracy back in exchange for a bounded,
known violation.</p>
  <ul>
  <li><b>Ticked</b> (the default): the fit is perturbed until it reaches the target. At
  ...
  raw fit sits at 1.083 with RMS 8.7e-04, a target of 1.00 gives 0.9999 at 2.6e-03, and a
  target of 1.05 gives 1.0499 at 1.3e-03, half the accuracy cost. A target above what the
  ...
  target field greys out at <i>1.00</i>, since nothing is aiming at it, and
...
number falls back to <i>1.00</i>. Reasonable targets: <i>1.00</i> for a transient or a
...
<p><b>A target is not always reachable.</b> The perturbation only moves the model's
residues, so a violation band that runs to infinity, caused by a non-passive constant
term, cannot be corrected at any target. <tt>tline_100um_ihp-sg13g2.s2p</tt> at order 13
...
higher target, since order 6 brings the same file to 1.018.</p></li>
...
&sigma;<sub>max</sub> against the target it was judged at (green inside the target, red
outside it), and the model order (universal) or the extraction frequency (structure).
Passivity reads <i>passive</i> below 1, <i>within target</i> when a raised target was
reached, <i>near-passive</i> when enforcement ran and could not reach it, and <i>not
passive</i> when the model is above its target. Structure models are passive by
