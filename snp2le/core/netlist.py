...
# Resistor thermal noise, switched off for the universal macromodel only.
#
# A vector-fit realisation has no physical resistors: scikit-rf builds it from state
# self-resistors and port sensor resistors that exist to reproduce the fitted response.
# Charging 4kTR against them is meaningless, and the number is not small.  On the
# committed 2-port it put 1.7e3 V of integrated output noise into a 50 ohm load, seven
# decades above the 2.2e-4 V of the terminations alone.  Worse, element count and value
# follow the pole count, so the same EM data fitted at order 6 and order 13 reports two
# different noise figures.
#
# Structure models are the opposite case and keep their noise.  Their resistors ARE
# devices: the Wilkinson isolation resistor is a real 100 ohm resistor on the die, Rs is
# a coil's conductor loss, the shunt R its substrate loss.  Silencing those understates
# a divider's or an LNA match's noise (measured: the Wilkinson's Riso alone contributes
# 1.5e-4 V integrated over 1 to 300 GHz).  ir.physical is the discriminator.
#
# Both simulators spell the switch the same way: ngspice takes "noisy=0" as a resistor
# instance parameter, and VACASK's devices/resistor.va declares
# `parameter integer noisy = 1` with type="instance".  Verified in IIC-OSIC-TOOLS on
# ngspice 47 and vacask bf59752: on a 1k/1k divider, switching the shunt resistor off
# drops total output noise by exactly sqrt(2) in ngspice and output PSD by exactly 2 in
# VACASK.  Adding the flag leaves the AC response bit-identical.
#
# Caveat worth knowing: a silenced macromodel is fully noiseless, so it no longer carries
# the thermal noise a lossy passive really does have at temperature.  Noise budgets
# through one of these models must account for the structure's loss separately.
_NOISY_OFF = "noisy=0"
...
def render_ngspice(ir: CircuitIR) -> str:
    ...
    noiseless = not ir.physical                       # fit artifacts, not real devices
    for e in ir.elements:
        if e.kind == "R" and noiseless:
            L.append(f"{e.name} {e.nodes[0]} {e.nodes[1]} {_num(e.value)} {_NOISY_OFF}")
        elif e.kind in ("R", "C", "L", "V"):
...
def render_vacask(ir: CircuitIR) -> str:
    ...
    noiseless = not ir.physical                       # fit artifacts, not real devices
    for e in ir.elements:
        L.append("  " + _vc_instance(e, noiseless))
