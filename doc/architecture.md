...
* Passivity is also reported as a number. `universal.max_singular_value()` returns
  ...
  `passivity_test()` reports. `ConverterState.passivity_target` (1.0 to 1.2) is the
  ...
  was. A target the residue perturbation cannot reach (an unbounded violation band,
  ...
  (`universal.clamp_passivity_target()`), so the GUI field answers "what is the limit"
  by showing it. The rule that the target applies only while enforcing lives in
  `universal.effective_target()` alone, so the GUI, the CLI and the engine cannot
  disagree about it.
